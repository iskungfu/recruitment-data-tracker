...
> 版本：v1.0 | 日期：2026-09-22 | 作者：软件架构师
...
│  - boss.py        │ │  - stats.py │ │  - renderer.py     │
│  - jobui.py       │ │  - wordfreq │ │  - templates/      │
│  - base.py (ABC)  │ │  - salary.py│ │  - charts.py       │
└──────────┬────────┘ └──────┬──────┘ └────────┬──────────┘
           │                 │                  │
┌──────────▼─────────────────▼──────────────────▼────────────┐
│                     storage/                                 │
...
┌──────────────────────▼──────────────────────────────────────┐
│                     utils/                                   │
│  anti_crawl.py │ logger.py │ http.py │ retry.py              │
└─────────────────────────────────────────────────────────────┘
```
...
| `collectors/` | BOSS / 职友集页面抓取 + 解析 + 入 `raw_jobs` 临时表 | `storage.dao`, `core.models`, `utils.anti_crawl` |
...
| `utils/` | 反爬、日志、HTTP 客户端、重试 | `core.config` |
...
[BOSS 直聘 API/页面]
       │
       ▼
[collectors/boss.py]  ──解析──→  raw_job dict
       │
       ▼
[core/cleansing.py]   ──清洗──→  cleaned job (城市名统一、学历归一)
       │
       ▼
[core/dedup.py]       ──去重──→  按 (job_id, snapshot_date) 复合键
       │
       ▼
[storage/dao.py]      ──入库──→  SQLite job_snapshot 表（每页立即 commit）
       │
       ▼  (Week 3)
...
| 依赖 | 用途 | 选型理由 |
|------|------|------|
| `playwright` | 浏览器自动化（BOSS CDP 登录态） | 唯一直连 Chrome DevTools Protocol 的 Python 库 |
...
| NFR | 设计方案 | 验证方式 |
|-----|---------|---------|
| **可靠性** | 逐页提交 + INSERT OR IGNORE + Ctrl+C 安全 | 中断后续抓测试（Week 2） |
| **可维护性** | 模块化单体 + ABC 采集器基类 + dataclass 契约 | 新增平台只需实现 `CollectorBase` |
| **性能** | 采集瓶颈在网络 I/O，单线程 + 12-22 秒延迟即可；分析阶段 pandas 全量加载 4 表到内存（单季度 <5000 条，<5MB） | 采集耗时不敏感；分析 <30 秒 |
| **安全性** | Cookie 明文存储于本地 `~/.config/recruitment-tracker/`，不提交 Git；`settings.yaml` 不含密钥 | .gitignore 排除 /config/local/ |
| **可观测性** | `utils/logger.py` 输出 JSON 行格式；`cli/health.py` 输出数据完整性报告（每城每方向记录数、空值率） | 每周健康检查 cron 推送飞书 |
...
| 风险 | 已锁决策 | ADR |
|------|---------|-----|
| BOSS 反爬升级（账号封禁） | 小批次采集 + 跨多天/多会话；单账号单关键词单城 ≤2 页，批次间隔 ≥5 分钟 | ADR-011 |
| Cookie 失效 | 健康检查检测 → 飞书通知用户重新登录，不自动重试 | ADR-008 |
| 前两季度无同比 | 报告中明确标注"时间序列建立中"，不伪造数据 | ADR-010 |
| 职友集采集方式未预研 | Week 2 同步预研，先跑通 BOSS 主源 | ADR-013 |
| 日薪制岗位（实习/兼职） | min/max 置空 + salary_raw 保留原文；ERD 建议新增 `salary_unit` 字段 | - |
