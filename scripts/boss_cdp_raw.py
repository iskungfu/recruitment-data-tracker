...
多关键词并发采集（Week 6，列表采集 + 全局去重 + 单文件输出）:
  python scripts/boss_cdp_raw.py --keywords "后端,前端,算法工程师" --city 北京 --pages 3 --workers 6 --output data/raw/beijing_cs.json
  python scripts/boss_cdp_raw.py --keywords-file keywords.txt --city 北京 --workers 6
  （worker 4-10，默认 6；同一 worker 连续搜索间隔 uniform(3,7)s；
   登录墙/验证码任一 worker 命中即全员优雅收尾并退出码 1；需 pip install tqdm）
"""
...
# ============================================================
# 多关键词并发采集（--keywords / --keywords-file，Week 6）
# ============================================================
#
# 设计要点：
# - ThreadPoolExecutor：playwright/websocket 均为同步 API，线程模型最直接。
# - 每个 worker 线程独立调 scrape_list——scrape_list 内部自建 CDPSession
#   （独立 websocket）+ create_page_session（独立 page），天然线程隔离；
#   同一 worker 串行处理队列里的下一个关键词。关键词之间不跨 worker 共享 page，
#   与"复用 page"等效的反检测语义（每次新 tab 更像真人），且不动 scrape_list
#   的签名与清理逻辑，风险最小。
# - worker 的 print 通过 _ConcurrentPrinter 按线程抑制（tqdm 进度条在主线程），
#   避免 6 路日志交错；错误不丢——异常汇总到主线程统一报告。
# - 登录墙/验证码（LoginGateError）：任一 worker 命中即 stop_event 通知其他
#   worker 在"当前关键词边界"优雅收尾，主线程重新抛出，不挂死。

_CONCURRENT_TLS = threading.local()
_CONCURRENT_PRINT_LOCK = threading.Lock()
_real_print = builtins.print


def _concurrent_print(*args, **kwargs):
    """并发模式下的 print：worker 线程静默，主线程加锁输出。"""
    if getattr(_CONCURRENT_TLS, "quiet", False):
        return
    with _CONCURRENT_PRINT_LOCK:
        _real_print(*args, **kwargs)


def parse_keywords(text):
    """逗号分隔关键词解析：支持中英文逗号，去空白、去重（保序）。"""
    if not text:
        return []
    parts = re.split(r"[,，\n]", text)
    seen, out = set(), []
...
def load_keywords_file(path):
    """从文件读关键词：每行一个 或 逗号分隔（可混合）。"""
    with open(path, encoding="utf-8") as f:
        return parse_keywords(f.read())


def dedupe_jobs_global(jobs):
    """跨关键词全局去重：优先 encrypt_job_id，缺失回退 job_link → job_id → title。

    不同关键词可能搜出同一岗位（如"后端"与"Java"），合并后必须按岗位级
    唯一键去重；encrypt_job_id 是 BOSS 的岗位主键，最可靠。
    """
    seen, out = set(), []
    for j in jobs:
        key = (j.get("encrypt_job_id") or j.get("job_link")
               or j.get("job_id") or j.get("title") or "")
        if key in seen:
            continue
        seen.add(key)
        out.append(j)
...
def merge_keyword_results(results, keywords):
    """按 keywords 顺序合并各关键词结果并全局去重，返回 (merged_jobs, per_keyword_counts)。"""
    all_jobs, counts = [], {}
    for kw in keywords:
        jobs = (results.get(kw) or {}).get("jobs", [])
        counts[kw] = len(jobs)
        all_jobs.extend(jobs)
    return dedupe_jobs_global(all_jobs), counts


def _safe_filename(text):
    return re.sub(r"[^\w]+", "_", text).strip("_") or "kw"


def scrape_keywords_concurrent(keywords, city_input, max_pages, filters,
                               workers=6, cdp_port=DEFAULT_CDP_PORT,
                               output_path=None, fmt="json",
                               allow_dom_fallback=False,
                               scrape_fn=None, inter_delay=None,
                               progress=True):
    """ThreadPoolExecutor 并发采集多关键词，返回 {keyword: list_data}。

    参数
    - workers：并发线程数（调用方已钳制到 4-10）。
    - scrape_fn：可注入的采集函数 fn(keyword) -> list_data（测试用）；
      默认包装 scrape_list（worker 线程静默 + 每关键词独立落盘到
      ``<output_stem>_parts/``，异常退出也能保留已抓部分）。
    - inter_delay：同一 worker 连续两次搜索的间隔秒数；None=uniform(3,7)
      （比串行版 12-22s 紧，因为 worker 间天然错开）。
    - progress：显示 tqdm 进度条（主线程，postfix 实时显示各 worker 当前关键词）。

    异常
    - 任一 worker 抛 LoginGateError（登录墙/验证码）→ 置 stop_event，其他
      worker 在当前关键词边界收尾，主线程重新抛出 LoginGateError。
    - 单关键词的其他异常不拖垮整体：记录到该关键词结果的 ``error`` 字段，
      total=0，继续后续关键词。
    """
    user_scrape_fn = scrape_fn
    parts_dir = None
    if user_scrape_fn is None and output_path:
        stem = os.path.splitext(os.path.basename(output_path))[0]
        parts_dir = os.path.join(os.path.dirname(output_path) or ".",
                                 stem + "_parts")
        os.makedirs(parts_dir, exist_ok=True)

    def default_scrape(kw, kw_output):
        _CONCURRENT_TLS.quiet = True
        try:
            return scrape_list(kw, city_input, max_pages, filters, kw_output,
                               cdp_port=cdp_port, fmt=fmt,
                               allow_dom_fallback=allow_dom_fallback)
        finally:
            _CONCURRENT_TLS.quiet = False

    work_q = Queue()
    for idx, kw in enumerate(keywords, 1):
        kw_output = None
        if parts_dir:
            kw_output = os.path.join(parts_dir, f"{idx:02d}_{_safe_filename(kw)}.json")
        work_q.put((kw, kw_output))
    ...
    if progress:
        try:
            ...
            pbar = tqdm(total=len(keywords), desc="并发采集", unit="词",
                        dynamic_ncols=True)
        except ImportError:
            log.warning("未安装 tqdm（pip install tqdm），进度条降级为纯文本")
            progress = False

    def _refresh_postfix():
        if pbar is not None:
            with state_lock:
                cur = " | ".join(f"w{w}:{k}" for w, k in sorted(active.items()))
            pbar.set_postfix_str(cur)

    def worker(wid):
        while not stop_event.is_set():
            try:
                kw, kw_output = work_q.get_nowait()
            except Empty:
                return
            with state_lock:
                active[wid] = kw
            _refresh_postfix()
            try:
                if user_scrape_fn is not None:
                    data = user_scrape_fn(kw)
                else:
                    data = default_scrape(kw, kw_output)
                with state_lock:
                    results[kw] = data
            except LoginGateError as e:
                with state_lock:
                    gate_errors.append(e)
                stop_event.set()
                return
            except Exception as e:  # noqa: BLE001 — 单关键词失败不拖垮整体，记 error 后继续
                with state_lock:
                    results[kw] = {
                        "keyword": kw, "total": 0, "jobs": [],
                ...
                if pbar is not None:
                    pbar.update(1)
                _refresh_postfix()
            # 同一 worker 连续两次搜索的间隔（登录墙收尾时不再等待）
            if not stop_event.is_set() and not work_q.empty():
                delay = inter_delay if inter_delay is not None else random.uniform(3, 7)
                if delay > 0:
                    time.sleep(delay)

    old_print = builtins.print
    builtins.print = _concurrent_print
    try:
        n_workers = max(1, min(workers, len(keywords)))
...
def run_concurrent(keywords, args, filters):
    """并发模式主流程（main 中调用），返回进程退出码。"""
    city_name, _ = resolve_city(args.city)
    output_path = args.output or default_output_path("jobs")
    workers = args.workers
    if workers < 4 or workers > 10:
        clamped = min(10, max(4, workers))
        print(f"⚠️ worker 数 {workers} 超出范围 4-10，已调整为 {clamped}")
        workers = clamped

    print("=== BOSS直聘并发采集 ===")
    print(f"关键词: {len(keywords)} 个 | 城市: {city_name} | 页数: {args.pages} | worker: {workers}")
    print()

    started = time.time()
    try:
        results = scrape_keywords_concurrent(
            keywords, args.city, args.pages, filters,
            workers=workers, cdp_port=args.cdp_port,
            output_path=output_path, fmt=args.format,
    ...
    merged, counts = merge_keyword_results(results, keywords)
    failed = [kw for kw in keywords if (results.get(kw) or {}).get("error")]
    ...
    meta = {
        ...
        "per_keyword": counts,
        "dedup": "encrypt_job_id",
    }
    flush_jobs(output_path, meta, merged)

    print("\n" + "=" * 60)
    print(f"并发采集完成: {len(keywords)} 个关键词, 原始 {sum(counts.values())} 条, "
          f"全局去重后 {len(merged)} 条, 耗时 {elapsed:.0f}s")
    for kw in keywords:
        mark = "✗" if kw in failed else "✓"
        print(f"  {mark} {kw}: {counts.get(kw, 0)} 条")
    if failed:
        print(f"\n⚠️ {len(failed)} 个关键词失败（结果中 error 字段有原因），其余正常")
    print(f"已保存: {output_path}")
...
示例:
  ...
  %(prog)s --setup-chrome
    ...
    p.add_argument("--keywords", default=None,
                   help="多关键词并发采集：逗号分隔列表（与 --keywords-file 二选一），"
                        "如 --keywords \"后端,前端,算法工程师\"")
    p.add_argument("--keywords-file", default=None,
                   help="多关键词并发采集：从文件读取（每行一个或逗号分隔）")
    p.add_argument("--workers", type=int, default=6,
                   help="并发 worker 数（默认 6，范围 4-10，超出自动钳制）")
    ...
    # 多关键词并发模式（--keywords / --keywords-file）：列表采集 + 全局去重 + 单文件输出
    kw_list = None
    if args.keywords and args.keywords_file:
        print("❌ --keywords 与 --keywords-file 只能二选一")
        sys.exit(1)
    if args.keywords:
        kw_list = parse_keywords(args.keywords)
    elif args.keywords_file:
        try:
            kw_list = load_keywords_file(args.keywords_file)
        except OSError as e:
            print(f"❌ 无法读取关键词文件 {args.keywords_file}: {e}")
            sys.exit(1)
    if kw_list is not None:
        ...
        if args.merge or args.input:
            print("❌ 并发模式不支持 --merge / --input")
            sys.exit(1)
        if args.detail:
            print("ℹ️  并发模式当前只做列表采集，自动忽略详情抓取（等价 --no-detail）")
        sys.exit(run_concurrent(kw_list, args, filters))
